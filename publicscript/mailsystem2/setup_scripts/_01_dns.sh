...
do
  ...
  usacloud dns record-add -y --name _dmarc --type TXT      --value "v=DMARC1; p=reject; rua=mailto:root@${domain}" ${domain}
...
usacloud ipv4 ptr-add -y --hostname ${FIRST_DOMAIN} ${IPADDR}
