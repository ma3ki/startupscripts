...
do
  ...
  usacloud dns record-add -y --name _dmarc --type TXT      --value "v=DMARC1; p=reject; rua=mailto:root@${domain}"
  usacloud dns record-add -y --name _adsp._domainkey --type TXT --value "dkim=discardable"
...
do
  ...
  usacloud dns record-add -y --name _dmarc.${name} --type TXT      --value "v=DMARC1; p=reject; rua=mailto:root@${domain}"
  usacloud dns record-add -y --name _adsp._domainkey.${name} --type TXT --value "dkim=discardable"
done
