...
yum install -y expect telnet bind-utils jq mailx git dstat sysstat bash-completion

# git repository
git clone https://github.com/ma3ki/startupscripts.git
source startupscripts/publicscript/mailsystem/config.source
cd startupscripts/publicscript/mailsystem
